<?php
/**
 * Template Name: Irish Courses Page
 */

// set data query ge_courses table - course details
$course_query = 'SELECT c.id, c.name, cr.region, ct.type, c.length, c.par, c.top_course, c.img_logo FROM gi_courses AS c INNER JOIN gi_course_types AS ct ON c.type = ct.id INNER JOIN gi_course_regions AS cr ON c.region = cr.id';

// query string to fetch course region and type data
$region_query = 'SELECT id, region FROM gi_course_regions';
$type_query = 'SELECT id, type FROM gi_course_types';
...
<div id="gs-wrapper">
	<section id="gs-content" class="clearfix">
		<div id="gs-search-form-container">
			<form id="gs-search-form" action="irish-courses" method="get">
